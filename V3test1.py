  ...
  if DETECT.value:
    try:
        ...
        if R_24V.is_active:
            ...
            log_result("fout", "24V is niet aangesloten aan de opstelling.")
            return False
...
def stap_5():
    try:
        ...
        if LED_GREEN2_OUT.value and p2c_ok and p4c_ok and not p2b_ok and p4b_ok:
            ...
            if not p2c_ok:
                foutmelding += "Relais 1 is niet open.controle op K2"
            if p2b_ok:
                foutmelding += "Relais 1 is niet open.controle op K2"
            if not p4c_ok:
                foutmelding += "Relais 2 is niet open.controle op K1"
            if not p4b_ok:
                foutmelding += "Relais 2 is niet open.controle op K1"
            log_result("fout", foutmelding.strip())
            return False
    ...
    try:
        ...
        if groen_status and rood_status and not p2c_ok and p4c_ok and not p2b_ok and p4b_ok:
            ...
            if p2b_ok:
                foutmelding += "Relais 1 is niet open.controle op K2"
            if not p4c_ok:
                foutmelding += "Relais 2 is open.controle op K1"
            if not p4b_ok:
                foutmelding += "Relais 2 is open.controle op K1"
            log_result("fout", foutmelding.strip())
            return False
...
def stap_7():
    try:
        ...
        if groen_status and geel_status and p2c_ok and not p4c_ok and p2b_ok and not p4b_ok:
            ...
            if not p2c_ok:
                foutmelding += "Relais 1 is open.controle op K2"
            if not p2b_ok:
                foutmelding += "Relais 1 is open.controle op K2"
            if p4c_ok:
                foutmelding += "Relais 2 is niet open.controle op K1"
...
def stap_14():
    try:
        ...
        succesbericht = f"{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | Optie : RISP REL | Serienummer: {SERIENUMMER} | Status: correct - PCB"
